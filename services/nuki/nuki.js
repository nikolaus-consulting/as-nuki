...
var localip = config.webserver.ip || nets.eth0[0].address
var localport = config.webserver.port
var bridges = []
...
async function addCallback(ip) {
    url = "http://" + localip + ":" + localport + "/nuki"
    console.log(url)
...
async function removeCallback(ip) {
	...
	//let response = await getRequest('http://' + ip + ':8080/callback/remove?id=' + 2 + '&' + parameter())
    if (index >= 0) {
